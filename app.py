...
<div class="app">
  ...
  <div class="glass hero">
    ...
    <button class="btn btn-ghost" onclick="vibe(30); getDirectUrl()" style="margin-top:8px">🔗 Get Direct URL (Phone Download)</button>
...
function getDirectUrl() {
  ...
  .then(function(d) {
    if (d.error) {
      document.getElementById("status").innerHTML = "<span style='color:#ff4d4d'>Error: " + d.error + "</span>";
      toast('Direct URL failed');
    } else {
      document.getElementById("status").innerHTML = "Direct URL ready!<br><a href='" + d.video_url + "' target='_blank' style='color:#00f5ff'>Click here to download</a>";
      navigator.clipboard.writeText(d.video_url);
      toast('URL copied to clipboard!');
    }
  })
  .catch(function() { toast('Direct URL fetch failed'); });
}
