...
input,select,button{ width:100%; padding:12px; margin-top:10px; border:none; border-radius:10px; } button{ background:#ff0055; color:white; font-weight:bold; } 
.progress{ background:#333; height:20px; border-radius:10px; overflow:hidden; margin-top:10px; } .bar{ height:20px; width:0%; background:#00ff9d; } 
img,video{ width:100%; border-radius:10px; margin-top:10px; } .card{ background:#111; padding:10px; border-radius:10px; margin-top:10px; } a{color:#00ff9d} 
</style></head>
<body>
<div class="box">
...
<div id="info"></div>
<select id="quality"> <option value="360">360p</option><option value="480">480p</option><option value="720">720p</option><option value="1080">1080p</option></select>
<select id="type"> <option value="video">Video</option><option value="audio">MP3</option></select>
...
document.getElementById("status").innerHTML= "Progress: "+d.percent+ "<br>Size: "+d.size+"<br>Speed: "+d.speed+"<br>ETA: "+d.eta
if(d.file!=""){ document.getElementById("result").innerHTML= "<video controls src='/file/"+d.file+"'></video><br><a href='/file/"+d.file+"' download>Download File</a>" }
})
},1000)
}
function history(){
...
def info():
    ...
    j=json.loads(data)
    return jsonify({"title":j.get("title",""), "channel":j.get("channel",""), "thumbnail":j.get("thumbnail","")})
...
def run_download(url,quality,typ):
    ...
    if typ=="audio":
        ...
        cmd=["yt-dlp","-f",f"bestvideo[height<={quality}]+bestaudio/best","--merge-output-format","webm","-o",SAVE_DIR+"/%(title)s.%(ext)s",url]
